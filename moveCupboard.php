...
function moveCupboard(){
  ...
  $ready_to_work = !$is_hungry && ($wearing_glasses xor $wearing_contacts) && !$needs_to_pee;
  if ($location !== "kitchen"){
    echo "You don't see a cupboard here!\n";
  } elseif ($moved_cupboard) {
    echo "You've already moved the cupboard!\n";
  } elseif (!($ready_to_work)) {
    echo "You're not ready to work! You need to be properly fed, have an empty bladder, and have corrected vision. Without these things, there's no point in even trying to move the cupboard.\n";
  } else {
